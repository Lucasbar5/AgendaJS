...
async function controlar_armazenar(contatos, contato) {
    // modelo
    await armazenar(contatos, contato);
    mostrar_mensagem('cadastrado');
    controlar_lista(contatos);
}

async function controlar_lista(origem) {
    let contatos = await listar(origem);
...
async function controlar_formulario_edicao(contatos, id) {
    let contato_antigo = await buscar(contatos, id);
    mostrar_formulario(contato_antigo, id, contatos, controlar_editar);
}

async function controlar_editar(contatos, contato, id) {
    // modelo
    await editar(contatos, id, contato);
...
async function controlar_exclusao_apagar(contatos, id) {
    if (confirm('Deseja apagar o id ' + id + '?')) {
        await apagar(contatos, id);
        controlar_lista(contatos);
    }
}
